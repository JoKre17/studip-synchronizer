...
public class ConfigManager {
	...
	private void saveProperties() throws Exception {
		...
		PrintWriter pw = new PrintWriter(new OutputStreamWriter(new FileOutputStream(configFile), StandardCharsets.UTF_8));
	...
	private void setListeners() {
		...
		downloadEnabledProperty.addListener(new ChangeListener<Boolean>() {
			...
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
				...
				if(newValue) {
					...
					if(synchronizationIntervalProperty.get() > 0 && downloadDirectory.exists() && downloadDirectory.isDirectory()) {
						SynchronizerApp.studipClient.getCourseService().getDownloadManager().setDownloadDirectory(downloadDirectory);
						SynchronizerApp.synchronizerTimer = new SynchronizeTimer(SynchronizerApp.studipClient, synchronizationIntervalProperty.get() * 60000);
		...
		downloadDirectoryPathProperty.addListener(new ChangeListener<String>() {
			...
			public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
				...
				SynchronizerApp.studipClient.getCourseService().getDownloadManager().setDownloadDirectory(new File(newValue));
				try {
					saveProperties();
				} catch (Exception e) {
					e.printStackTrace();
